...
use crate::models::{AetherConfig, ConnectionStatus};
...
impl Provider for AetherProvider {
    ...
    fn description(&self) -> &str {
        "Serverless anti-censorship tunnel (MASQUE/WireGuard/gool)"
    }
    ...
    async fn connect(
        ...
        let cfg = match config {
            crate::models::ProviderConfig::Aether(c) => c,
            _ => {
                return Err(crate::error::XtunnelError::Config(
                    "Invalid config for Aether".into(),
                ))
            }
        };
        ...
        *self.process.lock().await = Some(child);
        ...
        // Update state
        *state.provider_socks_port.lock() = self.socks_port;
        *state.current_provider.lock() = "aether".into();

        // Start tunnel
        crate::tunnel::start_tunnel(pids, self.socks_port, &state, app.clone()).await?;
